...
export const useDashboardKPIs = () => {
  ...
  // Revenue chart data - group payments by month
  const chartData = (() => {
  ...
  // Recent activity - combine leads + payments
  const recentActivity = (() => {
    ...
    payments?.slice(0, 5).forEach((p) => {
      items.push({
        ...
        desc: `${p.description ?? "Pagamento"} — €${Number(p.amount).toLocaleString()}`,
